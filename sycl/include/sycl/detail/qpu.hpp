...
// Verificar si el dispositivo es una QPU simulada
inline void ensure_qpu_device(const sycl::device &dev) {
  if (!dev.is_qpu()) {
    throw std::runtime_error(
        "This function can only be used with a QPU simulated device.");
  }
}
...
void apply_gate(sycl::queue &queue, std::complex<float> *stateVector_d,
                const unsigned int numStates, const int target,
                const std::complex<float> A, const std::complex<float> B,
                const std::complex<float> C, const std::complex<float> D) {

  ensure_qpu_device(
      queue.get_device()); // Verificar que el dispositivo sea una QPU
  queue
      .parallel_for<class Gate>(
          sycl::range<1>(numStates),
          [=](sycl::item<1> item) {
...
void h(sycl::queue &queue, std::complex<float> *stateVector_d,
       const unsigned int numQubits, const int target) {
  ...
  apply_gate(queue, stateVector_d, std::pow(2, numQubits) / 2, target,
             A / std::sqrt(2.0f), B / std::sqrt(2.0f), C / std::sqrt(2.0f),
             D / std::sqrt(2.0f));
}
void z(sycl::queue &queue, std::complex<float> *stateVector_d,
  ...
  std::complex<float> D(-1.0f, 0.0f);
  apply_gate(queue, stateVector_d, std::pow(2, numQubits) / 2, target, A, B, C,
             D);
}

void get_proba(sycl::queue &queue, std::complex<float> *stateVector_d,
               const unsigned int numStates, float *probaVector_d) {

  ensure_qpu_device(
      queue.get_device()); // Verificar que el dispositivo sea una QPU

  queue
      .parallel_for<class Proba>(sycl::range<1>(numStates),
                                 [=](sycl::item<1> item) {
                                   int global_id = item.get_id(0);
                                   std::complex<float> amp =
                                       stateVector_d[global_id];
                                   probaVector_d[global_id] =
                                       std::abs(amp * amp);
                                 })
      .wait();
}

void measure(sycl::queue &queue, std::complex<float> *stateVector_d,
             int numQubits, int samples) {
  ...
  for (size_t i = 0; i < size; ++i) {
    std::cout << "State " << toBinary(i, numQubits) << ": " << arr[i]
              << std::endl;
  }
  delete[] probaVector;
}
...
void rx(sycl::queue &queue, std::complex<float> *stateVector_d,
        const unsigned int numQubits, const int target, const double angle) {
